...
for (p2 in 2:P2){
  for (k in 1:K){
  beta2[p2,k] ~ dnorm(gamma2*beta1[p2-1,k],10)
  }
}
...
a <-jags.samples(jags,c('beta0','beta1','beta2','gamma','gamma2'),100)
...
plot(apply(a$beta1[,,,1], c(1,2), mean)[,1], type = 'l')
plot(apply(a$beta2[,,,1], c(1,2), mean)[,1], type = 'l')
